...
*/
int PointCloudUtils::detectNearObject(cv::Mat& image, cv::Mat& pointCloud, std::vector<ObjectInfo>& objectInfos)
{
...
/*----------------------------
	...
	*/
void PointCloudUtils::showObjectInfo(std::vector<ObjectInfo>& objectInfos, cv::Mat& outImage)
{
...
* 参数 : iters	[in]	形态学处理次数
*/
void PointCloudUtils::imageDenoising(cv::Mat& img, int iters)
{
...
* 参数 : objectInfos	[out]	目标信息序列
*/
void PointCloudUtils::parseCandidates(cv::Mat& objects, cv::Mat& depthMap, std::vector<ObjectInfo>& objectInfos)
{
	// 提取物体轮廓
	//	Mat canny_output;
	std::vector<std::vector<cv::Point> > contours;	// 物体轮廓点链
	...
	//			AfxMessageBox(str);
	for (unsigned int objID = 0; objID < contours.size(); objID++)//得出每个轮廓的信息
	{
