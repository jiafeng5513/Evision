...
// Draw 6DOF message
void draw6DoF(cv::Mat image, cv::Mat trans, cv::Mat angle, bool good_measurement)
{
    cv::Scalar color[3] = { cv::Scalar(0, 0, 255) ,cv::Scalar(0, 255, 0) ,cv::Scalar(255, 0, 0) };
    if (good_measurement == false) {
        color[0] = cv::Scalar(0, 255, 255);
        color[1] = cv::Scalar(0, 255, 255);
        color[2] = cv::Scalar(0, 255, 255);
    }
    cv::putText(image, "tx:" + FloatToString(trans.at<double>(0, 0)), cv::Point(20, 400), fontFace, fontScale,  color[0], thickness_font, 8);
    cv::putText(image, "ty:" + FloatToString(trans.at<double>(1, 0)), cv::Point(200, 400), fontFace, fontScale, color[1], thickness_font, 8);
    cv::putText(image, "tz:" + FloatToString(trans.at<double>(2, 0)), cv::Point(380, 400), fontFace, fontScale, color[2], thickness_font, 8);

    cv::putText(image, "rz:" + FloatToString(angle.at<double>(0)), cv::Point(20, 450), fontFace, fontScale, color[2], thickness_font, 8);
    cv::putText(image, "ry:" + FloatToString(angle.at<double>(1)), cv::Point(200, 450), fontFace, fontScale, color[1], thickness_font, 8);
    cv::putText(image, "rx:" + FloatToString(angle.at<double>(2)), cv::Point(380, 450), fontFace, fontScale, color[0], thickness_font, 8);
}
...
cv::Mat rot2angle(const cv::Mat& rotationMatrix) {
    cv::Mat angle(3, 1, CV_64F);
    ...
    auto rz = atan2(m10, m00);
    auto ry = atan2(-m20, sqrt(pow(m20, 2) + pow(m22, 2)));
    auto rx = atan2(m21, m22);

    angle.at<double>(0) = rz;
    angle.at<double>(1) = ry;
    angle.at<double>(2) = rx;
...
void createCubePly(const std::vector<float> size, const std::string filename)
{
    // size is a list[3] for length,width,height
    std::ofstream plyfile;
    plyfile.open(filename, std::ios::out | std::ios::trunc);//写入截断模式
    //ply文件分为文件头,顶点,面定义
    plyfile << "ply" << std::endl;
    plyfile << "format ascii 1.0" << std::endl;
    plyfile << "element vertex 8" << std::endl;
    plyfile << "property float x" << std::endl;
    plyfile << "property float y" << std::endl;
    plyfile << "property float z" << std::endl;
    plyfile << "element face 12" << std::endl;
    plyfile << "property list uint8 int32 vertex_index" << std::endl;
    ...
    z:size[2] = V6-V2
    */
    float vertex[8][3] = { {0.0,0.0,0.0},{0.0,size[1],0.0},{size[0],0.0,0.0},{size[0],size[1],0.0},
                     {0.0,0.0,size[2]},{0.0,size[1],size[2]},{size[0],0.0,size[2]},{size[0],size[1],size[2]} };
    plyfile << vertex[0][0] <<" "<< vertex[0][1] <<" "<< vertex[0][2] << std::endl;    //    V0 [      0,      0,      0]
    plyfile << vertex[1][0] <<" "<< vertex[1][1] <<" "<< vertex[1][2] << std::endl;    //    V1 [      0,size[1],      0]
    plyfile << vertex[2][0] <<" "<< vertex[2][1] <<" "<< vertex[2][2] << std::endl;    //    V2 [size[0],      0,      0]
    plyfile << vertex[3][0] <<" "<< vertex[3][1] <<" "<< vertex[3][2] << std::endl;    //    V3 [size[0],size[1],      0]
    plyfile << vertex[4][0] <<" "<< vertex[4][1] <<" "<< vertex[4][2] << std::endl;    //    V4 [      0,      0,size[2]]
    plyfile << vertex[5][0] <<" "<< vertex[5][1] <<" "<< vertex[5][2] << std::endl;    //    V5 [      0,size[1],size[2]]
    plyfile << vertex[6][0] <<" "<< vertex[6][1] <<" "<< vertex[6][2] << std::endl;    //    V6 [size[0],      0,size[2]]
    plyfile << vertex[7][0] <<" "<< vertex[7][1] <<" "<< vertex[7][2] << std::endl;    //    V7 [size[0],size[1],size[2]]
    plyfile << "3 5 1 0"<<std::endl;
    plyfile << "3 5 4 0"<<std::endl;
    plyfile << "3 4 0 2"<<std::endl;
    plyfile << "3 4 6 2"<<std::endl;
    plyfile << "3 7 5 4"<<std::endl;
    plyfile << "3 7 6 4"<<std::endl;
    plyfile << "3 3 2 1"<<std::endl;
    plyfile << "3 1 2 0"<<std::endl;
    plyfile << "3 5 7 1"<<std::endl;
    plyfile << "3 7 1 3"<<std::endl;
    plyfile << "3 7 6 3"<<std::endl;
    plyfile << "3 6 3 2"<<std::endl;
