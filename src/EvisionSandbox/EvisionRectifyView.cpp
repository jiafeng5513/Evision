...
void EvisionRectifyView::onPush_SelectSrcFolder()
{
	...
	fileDialog->setWindowTitle(QStringLiteral("请选择待矫正图片所在的文件夹"));
...
{
	QFileDialog * fileDialog = new QFileDialog();
	fileDialog->setWindowTitle(QStringLiteral("请选择校正后的图片保存在哪里"));
	fileDialog->setFileMode(QFileDialog::Directory);
	if (fileDialog->exec() == QDialog::Accepted)
	{
...
void EvisionRectifyView::onStart()
{
	...
	{
		QMessageBox::information(this, QStringLiteral("警告"), QStringLiteral("检查参数的完整性"));
		return;
	}
	ui.progressBar->setValue(0);
